...
const Tasks = () => {
  ...
  let {tasks} = useSelector((state) => state.tasksSlice);
  let pendingTasks = tasks.filter(item => item.status === 'pending');
  let runningTasks = tasks.filter(item => item.status === 'running');
  let doneTasks = tasks.filter(item => item.status === 'done');  
